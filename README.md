# Kaspi AutoPrice v5.2

FastAPI-сервис для безопасного расчёта цен и публикации полного XML-каталога Kaspi.

## Главное в v5.2

- Исторический результат больше не выглядит текущей ошибкой: если конкуренты появились позже, страница показывает «Сейчас доступно» и предлагает пересчитать только этот товар.
- Ручное обновление конкурентов сразу пересчитывает один товар и ставит XML на обновление.
- Добавлено безопасное удаление товара из активного каталога, восстановление через фильтр «Архив», массовый выбор и удаление.
- Исправлен двойной подсчёт выбранных товаров из desktop/mobile-разметки.
- Форма добавления одного товара вынесена на основную страницу каталога.
...
git add .
git status
git commit -m "Kaspi AutoPrice v5.2 catalog and current competitor state"
git push
```
