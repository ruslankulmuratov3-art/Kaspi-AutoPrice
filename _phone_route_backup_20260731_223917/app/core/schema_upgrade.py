...
def run_safe_schema_upgrade(engine: Engine) -> None:
    ...
    autopilot_job_columns = {
        'worker_id': varchar_120,
        'recovery_count': 'INTEGER',
        'recovery_notice_pending': bool_type,
    }
    if _existing_columns(engine, 'autopilot_jobs'):
        for column, ddl_type in autopilot_job_columns.items():
            try:
                _add_column(engine, 'autopilot_jobs', column, ddl_type)
            except Exception:
                pass

    autopilot_item_columns = {
        'competitor_seller': varchar_255,
        'competitor_seller_id': varchar_255,
        'min_price': float_type,
        'max_price': float_type,
        'cost_price': float_type,
        'margin_percent': float_type,
        'step': float_type,
        'http_status': 'INTEGER',
        'retry_at': dt,
        'source_device': varchar_255,
        'xml_feed_id': varchar_120,
    }
    if _existing_columns(engine, 'autopilot_job_items'):
        for column, ddl_type in autopilot_item_columns.items():
            try:
                _add_column(engine, 'autopilot_job_items', column, ddl_type)
            except Exception:
                pass
    ...
    if _existing_columns(engine, 'xml_feed_versions'):
        for column, ddl_type in xml_columns.items():
            try:
                _add_column(engine, 'xml_feed_versions', column, ddl_type)
            except Exception:
                pass
